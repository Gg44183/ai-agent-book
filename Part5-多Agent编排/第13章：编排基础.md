...
三家公司的搜索完全独立，没有依赖关系。但单 Agent 只能一个接一个做：
...
```
时间线（多 Agent）：
0s  ──┬► Agent A: 搜索 Tesla ──► 20s
     ├► Agent B: 搜索 BYD   ──► 20s
     └► Agent C: 搜索 Rivian ──► 20s
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  1. DECOMPOSE（分解）     2. DISPATCH（分发）               │
│     ├── 理解用户意图          ├── 为子任务选 Agent          │
│     ├── 拆解为子任务          ├── 分配 Token 预算           │
│     └── 识别依赖关系          └── 设置超时和重试            │
│                                                             │
│  3. COORDINATE（协调）    4. SYNTHESIZE（综合）             │
│     ├── 管理执行顺序          ├── 收集各 Agent 结果         │
│     ├── 处理依赖关系          ├── 去重、过滤、排序          │
│     └── 监控执行状态          └── 生成统一回复              │
│                                                             │
...
```
用户请求: "对比分析 Tesla、BYD、Rivian 的电动车战略"
    │
    ▼
┌─────────────────────────────────────────────────────────────┐
│  Step 1: 任务分解                                           │
...
┌─────────────────────────────────────────────────────────────┐
│  Step 3: 执行                                               │
│  ├── Agent A、B、C 并行执行子任务 1、2、3                    │
│  └── 等待完成后，Agent D 执行子任务 4                        │
└─────────────────────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────────────────────┐
...
└─────────────────────────────────────────────────────────────┘
    │
    ▼
最终响应: "Tesla 专注高端市场... BYD 主攻性价比... Rivian 聚焦皮卡..."
```
...
```
     ...
     ↓               ↓
┌─────┐         ┌─────┐
│Task2│         │Task3│ (并行，都依赖 Task1)
└──┬──┘         └──┬──┘
   │               │
   └───────┬───────┘
           ↓
       ┌─────┐
       │Task4│ (等待 Task2 + Task3)
       └─────┘
```
